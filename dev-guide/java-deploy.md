...
<div class='explanation primary'>
  ...
  上面文件有注释了的内容需要根据实际部署情况进行修改，若更换了数据库同时也需要修改easyorm.dialect以及easyorm.default-schema配置的参数
</div>

#### 使用脚本打包
脚本在项目根路径下`build-and-push-docker.sh`，具体内容如下
```shell
#!/usr/bin/env bash
#此处需要改为自己需要推送的仓库地址
...
services:
  ui:
    image: registry.cn-shenzhen.aliyuncs.com/jetlinks-demo/jetlinks-ui-pro:2.0.0 #需要更改，更改为自己推送的镜像仓库
    container_name: jetlinks-pro-ui
    ports:
      - 9000:80
    environment:
      - "API_BASE_PATH=http://192.168.66.171:8844/" #API根路径，需要更改，改为部署的后端ip:后端端口号
    volumes:

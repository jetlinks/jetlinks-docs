...
7. 拓展`spring.r2dbc.pool`相关配置.
8. 修复存在同类名协议包会出现冲突的问题.
9. 修复通知模版表达式中,表达式内容超过128引起数组下标越界问题.
10. 邮件通知模版中表达式增加对html的处理,解决部分富文本编辑器会把表达式转为html.
11. 增加对`Elasticsearch`数据权限控制的查询条件支持(Pro).
12. 设备消息统计支持数据权限控制[查看说明](../dev-guide/micrometer.md#设备统计)(Pro).
